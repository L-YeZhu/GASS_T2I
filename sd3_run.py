...
def generate_images_sd3(
    ...
    output_dir="./results/sd3medium/cc12m",
    ...
    for i, (prompt, count) in enumerate(prompt_counts):
        ...
        if all((subdir / f"sample_{j:02d}.png").exists() for j in range(count)):
            print(f"[{i+1}/{len(prompt_counts)}] Skipped (already exists): {prompt}")
            continue

        print(f"[{i+1}/{len(prompt_counts)}] Generating: {prompt}")
        start = time.time()
